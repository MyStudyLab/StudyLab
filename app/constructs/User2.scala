...
/**
  ...
  */
case class User2(username: String, contactInfo: ContactInfo, password: String, joined: Long,
...
object User2 {
  ...
  implicit val UserHandler = Macros.handler[User2]

  // A MongoDB projector to get only the fields for this class
  val projector = BSONDocument(
    "username" -> 1, "contactInfo" -> 1, "password" -> 1, "joined" -> 1,
